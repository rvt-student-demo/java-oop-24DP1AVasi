...
public class Sql {
    ...
    private static final String CSV_PATH = "src/main/java/todoList/todo.csv";
    ...
    private static void insertDataFromCSV(Connection conn) {
        try {
    ...
    public static void exportToCSV() {
        try (Connection conn = getConnection();
            ...
            while (rs.next()) {
                writer.println(rs.getInt("id") + ", " + rs.getString("task"));
            }
    ...
    public static void removeById(int id) {
        try (Connection conn = getConnection()) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(deleteSql)) {
                ...
                System.out.println(rowsDeleted > 0 ? "Task deleted successfully!" : "No task found with id: " + id);
            }
        } catch (SQLException e) {
    ...
    // ===== Instance Methods for UserInterface =====
    ...
    public Object[][] getAllTasksAsArray() {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            int index = 1;
            while (rs.next()) {
                data.add(new Object[]{index, rs.getString("task")});
                index++;
            }
        } catch (SQLException e) {
            System.out.println("Error retrieving tasks: " + e.getMessage());
